...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    /* ── Multiselect pills — override dark default ── */
    ...
    [data-testid="stExpander"] summary:hover { background: #F8F9FF !important; border-radius: 10px !important; }
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    # ══ OUTLIER UTILITY — fixed threshold ════════════════════════════════════════
    # ══════════════════════════════════════════════════════════════════════════════
    ...
    def parse_date(s):
        ...
        if m:
            try: return pd.to_datetime(m.group(1), format='%a %b %d %Y %H:%M:%S')
            except: pass
    ...
    def map_status(s):
        ...
        return {'finished':'Tenu','canceled':'Annulé','rejected':'Annulé','accepted':'Tenu',
                'tenu':'Tenu','annulé':'Annulé','annule':'Annulé'}.get(str(s).strip().lower(), str(s).strip())

    def map_space(s):
        if pd.isna(s) or str(s).strip() == '': return ''
        return {'Salle Polyvalente':'Salle de formation','ESPACE FONDATION ':'Salle Fondation',
                'Training Room':'Salle de formation','Terrasse':'Terrasse','BALE':'BALE',
                'Podcast Tunisia':'Podcast','Salle Design Thinking':'Salle Design Thinking',
                'Salle de réunion 113':'Salle de réunion 113'}.get(str(s).strip(), str(s).strip())
    ...
    def load_and_clean(file_bytes, file_name):
        ...
        df_raw = (pd.read_csv(io.BytesIO(file_bytes), dtype=str) if ext == '.csv'
        ...
        def gv(row, col): return row[col] if col and col in row.index else None

        records = []
        for _, row in df_raw.iterrows():
            fn = gv(row,COL_FIRSTNAME) or ''; ln = gv(row,COL_LASTNAME) or ''
            vis = gv(row,COL_VISIBILITY)
            activity_type = ('Evénement externe' if vis and str(vis).lower()=='public'
            ...
            records.append({
                ...
                'signed_declaration':('Oui' if pd.notna(policy_val) and str(policy_val).strip()
        ...
        activity_map = {
            'événement interne':'Evénement interne','evénement interne':'Evénement interne',
            'evenement interne':'Evénement interne','evénement externe':'Evénement externe',
        ...
        df['duration_flag'] = df['duration_hours'].apply(lambda x: 'multi-day' if pd.notna(x) and x>24 else '')
        ...
        # ── detect participant outliers (threshold > 1000) ──
    ...
    def build_excel(df):
        ...
        export_cols = ['event_name','activity_type','start_time','end_time','room','booking_date',
                    'organizer_email','organizer_name','organization','participants',
                    'status','signed_declaration','comment']
        labels = ['Event Name','Activity Type','Start Time','End Time','Room','Booking Date',
                'Organizer Email','Organizer Name','Organization','Participants',
                'Status','Signed Declaration','Comment']
        for ci, lbl in enumerate(labels, 1):
            c = ws.cell(row=1, column=ci, value=lbl); c.font=hf; c.fill=hfi; c.alignment=ha
        ...
        for spec in [
            ...
            dv.sqref=spec[2]; ws.add_data_validation(dv)
        tbl = Table(displayName='Evenements', ref=f'A1:M{last_row}')
        tbl.tableStyleInfo = TableStyleInfo(name='TableStyleMedium2', showRowStripes=True)
        ws.add_table(tbl); ws.freeze_panes='A2'
        ...
        total=len(df); held=int((df['status']=='Tenu').sum()); canc=int((df['status']=='Annulé').sum())
    ...
    def c_activity(df):
        ...
        ax.set_xlabel('Count', color=MGRAY, fontsize=10); ax.invert_yaxis()
        plt.tight_layout(); return fig
    ...
    def c_hour(df):
        ...
        all_h = pd.Series(0, index=range(24)); all_h.update(hc)
        fig, ax = make_fig(11, 4.2)
        slot_colors = {range(0,6):'#7C3AED', range(6,9):'#D97706',
                    range(9,12):'#0284C7', range(12,14):'#D97706',
                    range(14,18):'#059669', range(18,21):'#E11D48', range(21,24):'#0D9488'}
        hour_palette = []
        for h in range(24):
            for rng, c in slot_colors.items():
                if h in rng: hour_palette.append(c); break
        max_v = all_h.max()
        colors = [INDIGO if v==max_v and max_v>0 else hour_palette[i]
    ...
    def c_top_rooms(df):
        ...
        ax.set_xlabel('Events', color=MGRAY, fontsize=10); ax.invert_yaxis()
        plt.tight_layout(); return fig
    ...
    def c_avg_participants(df):
        ...
        ax.set_xlabel('Avg Participants', color=MGRAY, fontsize=10); ax.invert_yaxis()
        plt.tight_layout(); return fig

    def c_cancel_rate(df):
        rs = df.groupby(['room','status']).size().unstack(fill_value=0)
        if 'Annulé' not in rs.columns: return None
        ...
        ax.set_xlabel('Cancellation Rate', color=MGRAY, fontsize=10); ax.invert_yaxis()
        plt.tight_layout(); return fig
    ...
    def c_room_hours(df):
        ...
        ax.set_xlabel('Total Hours', color=MGRAY, fontsize=10); ax.invert_yaxis()
        plt.tight_layout(); return fig
    ...
    def c_participants_dist(df):
        ...
        axes[0].boxplot(clean, vert=True, patch_artist=True,
        ...
        if n_out > 0:
            axes[1].axvline(PARTICIPANT_MAX, color=ROSE, linewidth=1.2, linestyle="--", alpha=0.7,
    ...
    def c_heatmap(df):
        order = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday']
        heat = (df.groupby(['weekday','hour']).size().unstack(fill_value=0).reindex(order, fill_value=0))
        fig, ax = plt.subplots(figsize=(14, 4.5), facecolor=WHITE)
        ax.set_facecolor(WHITE)
        cmap = LinearSegmentedColormap.from_list('indigo', [WHITE, '#EEF2FF', '#A5B4FC', INDIGO, '#312E81'])
    ...
    def c_yearly_trend(df):
        ...
        for ax in axes:
            ...
            ax.yaxis.grid(True, color='#F4F4F8', linewidth=0.7)
            ax.set_axisbelow(True)
        axes[0].bar(yearly['year'].astype(str), yearly['events'], color=yr_colors, edgecolor=WHITE, width=0.5, alpha=0.92)
    ...
    with st.spinner("Processing data…"):
        df, empty_rows, duplicate_rows, negative_rows = load_and_clean(uploaded.read(), uploaded.name)

    # participant stats — outliers excluded
    clean_participants = remove_outliers(df['participants'])
    n_outliers         = int(df['participant_outlier'].sum())
    lo_bound, hi_bound = iqr_bounds(df['participants'])
    ...
    # ── TAB 2: DATA ───────────────────────────────────────────────────────────────
    with tab2:
        show_cols = [c for c in ['event_name','activity_type','start_time','end_time',
        ...
        st.caption(f"{len(dff):,} rows · {len(show_cols)} columns shown · "
                f"participant_outlier = True means excluded from participant stats")
    ...
    with tab3:
        ...
        if not outlier_rows.empty:
            with st.expander(f"Participant outliers (> 1,000, kept but excluded from stats)  ·  {len(outlier_rows):,}"):
                st.dataframe(
        ...
        if not multi.empty:
            with st.expander(f"Multi-day events kept  ·  {len(multi):,}"):
                st.dataframe(multi[['event_name','start_time','end_time','duration_hours']], use_container_width=True)
    ...
    with tab4:
        ...
        with col1:
            ...
            st.dataframe(dff['duration_hours'].describe().rename('duration_hours').to_frame(),
                        use_container_width=True)
        with col2:
            st.markdown("**Participants (outliers excluded)**")
            st.dataframe(clean_dff.describe().rename('participants_clean').to_frame(),
                        use_container_width=True)
        ...
        st.markdown('<div class="section-label">Participants by Activity Type (outliers excluded)</div>', unsafe_allow_html=True)
        ...
        dur_by_room = dff[dff['room']!=''].groupby('room')['duration_hours'].agg(
            Total_Hours='sum', Avg_Hours='mean', Events='count'
        ).round(1).sort_values('Total_Hours', ascending=False)
        ...
        for col, icon, title, desc, color, btn_label, btn_key, data_fn, filename, mime in [
            ...
            with col:
                ...
                with st.spinner("Building…"):
                    file_data = data_fn()
                st.download_button(btn_label, data=file_data,
